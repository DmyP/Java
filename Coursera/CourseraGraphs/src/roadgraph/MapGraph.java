...
public class MapGraph {
    ...
    public boolean addVertex(GeographicPoint location)
    {
        if (location == null || nodes.containsKey(location))
            return false;
        return nodes.put(location, new MapNode(location)) != null;
    }
    ...
    public void addEdge(GeographicPoint from, GeographicPoint to, String roadName,
        ...
        if (Objects.isNull(from) ||  Objects.isNull(to) ||
                Objects.isNull(roadName) || Objects.isNull(roadType) || length < 0)
                                     ...
                                     GeographicPoint goal, Consumer<GeographicPoint> nodeSearched)
    {
        ...
        return pathGenerator(start, goal, parent);
    }

    /**  the Main logic of breadth first search
     *
     * @param start The starting location
     * @param goal The goal location
     * @param nodeSearched A hook for visualization.
     * @return Map of GeographicPoints that is a route from start to goal. NULL if start or goal is empty. Empty map if no way found
     */
    private Map<GeographicPoint, GeographicPoint> bfsSearch(GeographicPoint start,
        ...
        Queue<GeographicPoint> toVisit = new LinkedList<>();
        toVisit.add(start);
        visited.add(start);
        nodeSearched.accept(start);

        while (!toVisit.isEmpty()) {
            GeographicPoint current = toVisit.poll();
            if (current.distance(goal) == 0d)
                return parent;
            for (MapEdge e : nodes.get(current).getEdges()) {
     ...
     * @param start The starting location
     * @param goal The goal location
     * @param parent list of route visited nodes
     * @return The list of intersections that form the shortest (unweighted)
     *   path from start to goal (including both start and goal).
     */
    private List<GeographicPoint> pathGenerator(GeographicPoint start, GeographicPoint goal,
     ...
     * @param args no args for default
     */
